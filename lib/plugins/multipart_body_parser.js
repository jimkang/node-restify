...
function multipartBodyParser(options) {
  ...
  assert.optionalBool(options.maxFileSize, "options.maxFileSize");
  ...
  function parseMultipartBody(req, res, next) {
    ...
    if (options.maxFileSize) {
      form = new formidable.IncomingForm({ maxFileSize: options.maxFileSize });
    } else {
        ...
        if (options.mapFiles) {
          ...
          Object.keys(files).forEach(function(f) {
            ...
            fs.readFile(files[f].path, function(ex, data) {
              ...
              if (ex) {
                return next(
                  new errors.InternalError(ex, "unable to read file" + f)
                );
              }
              req.params[f] = data;
              return true;
